...
import { getBlockedAccounts, unblockAccount, type PageCursor } from '@/lib/mastodon';
...
export default function BlockedAccountsScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await getBlockedAccounts();
        if (cancelled) return;
        setAccounts(response.accounts);
        setCursor(response.next);
      } catch (error: any) {
        if (!cancelled) setErrorMessage(error.message || 'Could not load blocked accounts');
      } finally {
        if (!cancelled) {
          setLoading(false);
          setRefreshing(false);
  ...
  const loadMore = useCallback(async () => {
    ...
    setLoadingMore(true);
    try {
      const response = await getBlockedAccounts(cursor);
  ...
  const handleUnblock = useCallback(async (account: MastodonAccount) => {
    ...
    try {
      ...
      setErrorMessage(error.message || `Could not unblock @${account.acct}`);
    } finally {
      setPendingId(null);
    }
  }, []);
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <Stack.Screen
        options={{ title: 'Blocked accounts', headerShown: true, headerBackTitle: 'Back' }}
      />
      ...
      <AccountList
        ...
        emptyBody="Accounts you block will appear here, so you can undo it later."
        renderActions={account => (
          <TouchableOpacity
            style={[styles.button, { borderColor: theme.border }]}
            onPress={() => handleUnblock(account)}
            disabled={pendingId === account.id}
            accessible={true}
            accessibilityRole="button"
            accessibilityLabel={`Unblock ${account.displayName || account.username}`}
            accessibilityHint="Double tap to let them follow you and see your posts again"
