...
export default function BookmarksScreen() {
  ...
  useEffect(() => {
    console.log('BookmarksScreen mounted, loading bookmarks');
    loadBookmarks();
  }, []);

  const loadBookmarks = useCallback(async (maxId?: string) => {
    try {
      console.log('Loading bookmarks', maxId ? `with maxId: ${maxId}` : '');
      if (!maxId) {
        setLoading(true);
      }
      const response = await getBookmarks(maxId);
      console.log(`Loaded ${response.posts.length} bookmarked posts`);
      ...
      setErrorMessage(error.message || 'Failed to load bookmarks');
  ...
  const handleBookmark = async (postId: string, currentState: boolean) => {
    ...
    // Optimistic update - remove from list if unbookmarking
    if (currentState) {
